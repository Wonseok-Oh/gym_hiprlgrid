...
from find_frontier.srv import InitPosList, InitPosListResponse
...
#                print('placing object %s' %(obj.type))
    ...
    def __init__(self, grid_size_ = 20, max_steps_ = 100, agent_view_size_ = 5, num_objects=1, num_boxes = 10, process_num = 0, num_agents=1):
        ...
        for i in range(num_agents):
            self.agents_pose_pub_vis.append(rospy.Publisher("pose" + str(process_num) + str(i), PoseStamped, queue_size = 1, latch=True))
        ...
        self.individual_map_pub = rospy.Publisher("individual_map" + str(process_num), OccupancyGrid, queue_size = 1, latch=True)
...
#            self.spatial_map.rate.sleep()
    ...
    def explore_plan_cb(self, msg):
        ...
        for i in range(self.num_agents):
            ...
            if len(msg.list[j].data) < min_len and len(msg.list[j].data) > 1:
        ...
        for i in range(self.num_agents):
            if self.agents[i].mode != self.Option_mode.explore:
                continue
            if len(msg.list[j].data) == 1 and msg.list[j].data[0] < 0:
    ...
    def invoke(self, meta_action, render = False):
        ...
        for i in range(len(self.agents)):
            if len(self.agent[i].action_list) < min_option_len:
                min_option_len = len(self.agent[i].action_list)
    ...
    def generate_network_input_one_hot_for_mlp(self, obs):
        ...
        map = np.zeros(shape=(10, 10, 7), dtype=np.uint8)
...
#            self.agent_init_pos_pub.publish(self.agent_init_pos)
    ...
    def update_maps(self, obs, action):
        ...
        for i in range(len(self.agents)):
            ...
            if np.array_equal(self.agents[i].prev_pos, self.agents[i].pos) and np.array_equal(self.agents[i].prev_dir, self.agents[i].dir): # no movement
                if action[i] == self.Actions.open or action[i] == self.Actions.pickup or action[i] == self.Actions.drop:
    ...
    def explore(self, id_list):
        ...
        try:
            ...
            result = init_pose_update([self.agent_init_pos.poses[i].position.x for i in id_list],
                                      [self.agent_init_pos.poses[i].position.y for i in id_list], 
                                      [self.agent_init_dir[i] for i in id_list])
        ...
        for i in id_list:
            ...
            if self.agents[i].action_list[0] == self.Actions.done:
